...
4.  Set up PostgreSQL database:
    ...
    *   Log into PostgreSQL as a superuser (e.g., `postgres`):

        ```bash
        psql -U postgres
        ```

    *   Run the setup script (ensure that `setup_db.sql` file is in the same directory where the command is run or provide correct path):
